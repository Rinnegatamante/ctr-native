...
void VehPhysGeneral_JumpAndFriction(struct Thread *t, struct Driver *d)
{
	...
	if (((d->kartState != KS_DRIFTING) && ((d->actionsFlagSet & 0x800000) == 0)) && (VecY = 0, d->reserves == 0))
	{
	...
	CHECK_FOR_ANY_JUMP:
		...
		if (d->forcedJump_trampoline == 0)
		{
			if ((d->jump_CoyoteTimerMS == 0) || (d->jump_TenBuffer == 0) || (d->jump_CooldownMS != 0))
			{
				if ((d->actionsFlagSet & 1) != 0)
				{
					if ((d->underDriver != 0) && (tempValue = (int)d->underDriver->mulNormVecY, tempValue != 0))
					{
		...
		if (((d->stepFlagSet & 3) == 0) || (d->baseSpeed < 1))
		{
			if (d->baseSpeed != 0)
			{
				if ((((d->terrainMeta1->flags & 4) == 0) || (d->baseSpeed < 1)) || (-1 < d->speedApprox))
				{
					...
					if (((0x2ff < absSpeed) && ((d->baseSpeed) < 1 || (ApproxSpeed < 1))) && ((-1 < d->baseSpeed || (-1 < ApproxSpeed))))
						goto PROCESS_ACCEL;
				}
...
// og code
		...
		velocityMagnitude = VehCalc_FastSqrt(movement.vx * movement.vx + movement.vy * movement.vy + movement.vz * movement.vz, 0x10);
#endif
